...
def generate_music_with_acestep(
    ...
    *,
    ...
    def _fix_seq(seq):
        # 리스트 내부 원소 정규화 + ["문자", "숫자", 0] → ["숫자", 0]
        out = [_unhash(v) for v in seq]
        if len(out) == 3 and isinstance(out[0], str) and not str(out[0]).isdigit() and isinstance(out[1], (str, int)) and out[2] == 0:
            return [str(out[1]), 0]
        if len(out) == 2 and isinstance(out[0], str):
            return [str(_unhash(out[0])), out[1]]
        return out
    ...
    if isinstance(g, dict) and "nodes" not in g:
        for _nid, node in list(g.items()):
            if isinstance(node, dict) and isinstance(node.get("inputs"), dict):
                ...
                # TextEncodeAceStepAudio의 lyrics '#74' 보정
                if node.get("class_type") == "TextEncodeAceStepAudio":
    ...
    # ✅ 추가(2): 완료 후에도 한 번 더 덤프(서버가 수정/보정했는지 확인용)
