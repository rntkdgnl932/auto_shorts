...
from app.utils import load_json, save_json, sanitize_title, effective_title, save_to_user_library, audio_duration_sec, _submit_and_wait
...
def build_story_json(
    *,
    ...
    except (TypeError, ValueError, KeyError) as ae:
        _p(f"의미 단위 덮어쓰기 건너뜀(입력 형식 오류): {ae}")
    ...
    try:
        ...
        if errs:
            ...
            _p("v1.1 저장 및 검증 통과")
    except (ImportError, ModuleNotFoundError, OSError, ValueError) as te:
        # 실패 시라도 원본을 남김
        save_json(story_path, story_raw)
        _p(f"정규화 실패 → 원본 저장 ({type(te).__name__}: {te})")
    ...
    if also_build_docs and (_build_image_json or _build_movie_json):
        ...
        except (ImportError, ModuleNotFoundError, OSError, ValueError) as ze:
            _p(f"image/movie 문서 생성 실패: {type(ze).__name__}: {ze}")
