...
def generate_title_lyrics_tags(
    *,
    ...
    가사 생성:
      ...
      - 섹션 배치는 AI가 곡 길이에 맞춰 자연스럽게 결정(verse/chorus/optional bridge)
      - [intro] / [outro] 헤더는 반드시 포함하되 '무가사'로 둔다
      - 코러스는 최소 1회 이상 반복되도록 유도
    ...
    def _ensure_intro_outro_headers(text: str) -> str:
        """[intro]/[outro] 헤더가 없으면 추가하고, 해당 섹션에는 가사 줄을 두지 않는다."""
        lines = (text or "").splitlines()
        has_intro = any((ln.strip().lower().startswith("[intro]")) for ln in lines)
    ...
    system = (
        ...
        "- Allowed headers: [intro], [verse], [chorus], [bridge], [outro] (lowercase)\n"
        f"- Body line budget (EXCLUDING intro/outro lines): {min_lines}–{max_lines} total lines.\n"
        "- IMPORTANT:\n"
        "  1) [intro] and [outro] headers MUST exist, and they MUST have NO lyric lines under them (silent sections).\n"
        "  2) Let THE MODEL decide a natural section plan based on the total duration: e.g., "
        "[verse]/[chorus]/(optional [bridge]) repeated as fits 2–3 minutes.\n"
        "  3) Ensure at least one CHORUS repetition so the song feels like a song.\n"
        "  4) Keep only lyric lines under [verse]/[chorus]/[bridge].\n"
        "  5) Do NOT add production notes, camera/stage directions, or metadata.\n"
    ...
    # ---- 모델 호출 (AI.ask_smart는 ai.py에 존재해야 함)
    if prefer is None:
        prefer0 = "openai"
    else:
        prefer0 = str(prefer)
    ...
    # (선택) 섹션 라벨 정규화가 파일에 이미 있다면 활용, 없으면 통과
    try:
        # 같은 파일 내 normalize_sections가 있다면 사용
        from lyrics_gen import normalize_sections as _norm_sections  # type: ignore
        lyrics = _norm_sections(lyrics)
    except (ImportError, AttributeError):
        pass

    # [intro]/[outro] 강제 + 무가사 보장
    lyrics = _ensure_intro_outro_headers(lyrics)

    # 기본 노이즈 정리(너무 공격적이지 않게)
    section_only = re.compile(
        r"^\s*\[(?:intro|verse|pre[- ]?chorus|chorus|bridge|outro|hook|coda|break|tag|interlude)(?:\s+\d+)?\]\s*$",
        re.IGNORECASE,
    )
