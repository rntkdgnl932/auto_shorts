    ...
    *,
    ...
    try:
        ...
        for seg in segments:
            ...
            e = float(seg.end or max(s, 0.01))
            out_segs.append({"start": s, "end": e, "text": (seg.text or "").strip()})
            for w in (seg.words or []):
                if w.start is None or w.end is None:
                    continue
                out_words.append((float(w.start), float(w.end), (w.word or "").strip()))
