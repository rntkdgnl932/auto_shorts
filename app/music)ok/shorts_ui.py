...
import os
os.environ["CUDA_VISIBLE_DEVICES"] = ""   # PyTorch/onnx/CUDA 모두 비활성
os.environ["CT2_FORCE_CPU"] = "1"         # faster-whisper(CTranslate2) 강제 CPU
os.environ["CT2_USE_GPU"] = "0"           # (보조 안전장치)
os.environ["ONNXRUNTIME_FORCE_CPU"] = "1" # onnxruntime가 CUDA provider 안 쓰게
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def on_click_analyze_music(self) -> None:
        ...
        # 설정값(필요시 여기만 조정)
        USE_VOCAL_SEPARATION = False  # demucs 설치되어 있으면 True 추천
        MODEL_SIZE = "medium"  # small/medium/large-v3 등
        MIN_LEN = 0.5
        END_BIAS = 2.5
        ...
        # 필수 함수 확인
        reqs = ["get_audio_duration", "detect_onsets_seconds", "layout_time_by_weights", "prepare_pure_lyrics_lines",
        ...
        def done(ok: bool, payload, err):
            ...
            dlg.setWindowTitle("음악분석 결과 — Whisper 정렬 + 폴백 배분")
            dlg.resize(900, 700)
    ...
    def _build_ui(self):
        ...
        top.addWidget(self._group("가사", self.te_lyrics), 1)
    ...
    def _current_seconds(self) -> int:
        """
        UI 라디오 상태를 '초'로 반환한다.
        30초(테스트)는 30을, 1/2/3분은 60/120/180을 반환.
        """
        # 30초 라디오
        if hasattr(self, "rb_30s") and self.rb_30s.isChecked():
            return 30

        # 과거 호환: rb_20s / rb_time_20s 라디오가 남아 있을 때도 30으로 처리
    ...
    def _bind_length_radios(self) -> None:
        ...
        # 과거 20초 라디오가 UI에 남아있다면 동일하게 연결(반환은 30초로 처리됨)
    ...
    def _save_project_snapshot(self, title: str, lyrics: str, prompt: str) -> str:
        ...
        # utils
        try:
            from app.utils import save_json, load_json, create_project_files, sanitize_title
        except Exception:
            from utils import save_json, load_json, create_project_files, sanitize_title  # type: ignore
    ...
    def _apply_lyrics_result(self, data: dict, manual_title: str, prompt: str) -> None:
        ...
        try:
            ...
            from tag_norm import normalize_tags_to_english  # type: ignore
        try:
            from app.lyrics_gen import create_project_files
        except Exception:
        ...
        print("[TAGDBG] manual options:", sorted(allowed), flush=True)
        ...
        print("[TAGDBG] saved seconds:", meta.get("time"), "(target_seconds:", meta.get("target_seconds"), ")",
              flush=True)
