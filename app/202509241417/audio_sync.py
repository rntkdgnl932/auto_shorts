...
# ─────────────────────────────────────────────────────────────
# 4) Whisper 전사 — faster-whisper 우선, 실패 시 openai-whisper 폴백
# ─────────────────────────────────────────────────────────────
# app/audio_sync.py
...
def transcribe_words(
    ...
    *,
    ...
    - 항상 CPU 사용 (CUDA를 전혀 쓰지 않음)
    - faster-whisper 우선 사용, 실패 시 openai-whisper로 폴백
    - initial_prompt는 지원 시에만 전달, 길이/바이트 안전 절삭
    반환 형식:
      {
        "segments": [{"start": s, "end": e, "text": str}, ...],
        "words":    [(start, end, "token"), ...]
      }
    실패 시 {}.
    """
    import os
    ...
    # 힌트 문자열 정리: 공백 축약 + 토큰 80개 + 바이트 1000 제한
    def _trim_prompt(s: str) -> str:
        ...
        b = s.encode("utf-8")
        if len(b) > 1000:
            s = b[:1000].decode("utf-8", errors="ignore")
        return s
    ...
    try:
        ...
        # 인스턴스 메서드 시그니처 확인(버전별 파라미터 차이 대응)
        sig = inspect.signature(model.transcribe)

        # ★ kwargs를 Any로 선언해 타입 경고 제거
        kwargs: Dict[str, Any] = {
            "word_timestamps": True,
            "vad_filter": bool(vad_filter),
            "beam_size": int(max(1, beam_size)),
        }
        if initial_prompt and "initial_prompt" in sig.parameters:
            kwargs["initial_prompt"] = initial_prompt

        try:
            segments, _info = model.transcribe(p, **kwargs)
        except TypeError:
            # 어떤 버전에선 initial_prompt 미지원
        ...
        for seg in segments:
            ...
            e = float(getattr(seg, "end", s + 0.01) or (s + 0.01))
            out_segs.append({"start": s, "end": e, "text": (getattr(seg, "text", "") or "").strip()})
            for w in (getattr(seg, "words", None) or []):
                ws = getattr(w, "start", None)
                we = getattr(w, "end", None)
                if ws is None or we is None:
                    continue
                out_words.append((float(ws), float(we), (getattr(w, "word", "") or "").strip()))
        return {"segments": out_segs, "words": out_words}
    except Exception:
        # 폴백으로 진행
        pass

    # ── 2) openai-whisper 폴백 (CPU, fp16=False)
    try:
        ...
        result = model.transcribe(
            ...
            beam_size=int(max(1, beam_size)),
            best_of=max(int(max(1, beam_size)), 5),
