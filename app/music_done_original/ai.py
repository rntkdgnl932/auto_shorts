...
class AI:
    ...
    def _init_clients(self):
        """
        OpenAI/Gemini 클라이언트를 키가 있는 한 모두 준비한다.
        provider 값은 기본 선호 모드로만 사용되고, 실제 라우팅은 _ask()/ask_smart에서 수행.
        """
        # OpenAI
        self._openai = None
        if OpenAI is None:
            # 설치되지 않았으면 그냥 넘어감 (Gemini만으로도 동작 가능)
            pass
        else:
            key = self.cfg.openai_api_key or ""
            if key:
                if self.cfg.openai_base_url:
                    self._openai = OpenAI(api_key=key, base_url=self.cfg.openai_base_url)
                else:
    ...
    def ask_smart(
            ...
            user: str,
            *,
            prefer: str = "openai",  # "openai" | "gemini"
            allow_fallback: bool = True,  # True면 우선 실패 시 다른 쪽 시도
            trace=None,
            **kwargs,
    ) -> str:
        """
        공급자 우선순위에 따라 호출하고, 실제 호출한 쪽의 trace 라벨을 정확히 남긴다.
        prefer="gemini" & allow_fallback=False 이면 Gemini만 사용.
        """
